...
use App\Http\Controllers\Api\ManagerRequestController;
...
Route::prefix('')->name('.')->group(function () {
    ...
    Route::get('tracks/{track}/related', [TrackController::class, 'getRelatedTracks'])->name('tracks.related');

    // Donations (public - guests can donate)
    Route::post('donations/initiate', [DonationController::class, 'initiate'])->name('donations.initiate');

    // Manager Requests (public - guests can submit)
    Route::post('manager-requests', [ManagerRequestController::class, 'store'])
        ->middleware('throttle:5,1')
        ->name('manager-requests.store');
});
...
Route::prefix('')->name('.')->middleware('auth:sanctum')->group(function () {
    ...
    // Queue
    Route::get('queue', [TrackController::class, 'getQueue'])->name('queue.index');
    Route::post('tracks/{track}/queue', [TrackController::class, 'addToQueue'])->name('tracks.queue.add');
    Route::post('queue/reorder', [TrackController::class, 'reorderQueue'])->name('queue.reorder');
    Route::delete('queue/{queue}', [TrackController::class, 'removeFromQueue'])->name('queue.remove');
    Route::delete('queue', [TrackController::class, 'clearQueue'])->name('queue.clear');
    ...
    Route::prefix('playlists')->name('playlists.')->group(function () {
        ...
        Route::post('{playlist}/tracks/{trackId}', [PlaylistController::class, 'addTrack'])->name('tracks.add');
        Route::delete('{playlist}/tracks/{trackId}', [PlaylistController::class, 'removeTrack'])->name('tracks.remove');
    });
    ...
    // Feedbacks (authenticated users submit / view their own)
    Route::post('feedbacks',     [FeedbackController::class, 'store'])->name('feedbacks.store');
    Route::get('feedbacks/mine', [FeedbackController::class, 'mine'])->name('feedbacks.mine');
    ...
    Route::middleware([EnsureUserHasRole::class . ':admin'])->prefix('admin')->name('admin.')->group(function () {
        ...
        Route::get('manager-requests/{managerRequest}', [ManagerRequestController::class, 'show'])->name('manager-requests.show');
        Route::put('manager-requests/{managerRequest}', [ManagerRequestController::class, 'update'])->name('manager-requests.update');
        Route::delete('manager-requests/{managerRequest}', [ManagerRequestController::class, 'destroy'])->name('manager-requests.destroy');
